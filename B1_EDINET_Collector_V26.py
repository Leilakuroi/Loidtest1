...
class EdinetB1CategorizedTool:
    def __init__(self, root):
        self.root = root
        self.root.title("EDINET B1 Pipeline v26.0 - Japanese Categorized")
        ...
        # Report Categories for subfolder naming
        self.categories = [
    ...
    def setup_ui(self):
        ...
        tk.Label(config_card, text="API Key Settings:", font=UITheme.FONT_HEAD, bg=UITheme.BG_CARD).grid(row=0, column=0, sticky="w")
        ...
        tk.Label(search_card, text="Target Aliases (Incremental Scan):", font=UITheme.FONT_BODY, bg=UITheme.BG_CARD).pack(anchor="w", pady=(15, 5))
    ...
    def load_config(self):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                self.log("API Authentication Successful.")
                ...
                for orig_alias, keyword in processed_keywords:
                    if keyword in name_en or keyword in name_jp or keyword == ticker_5:
                        if ed_code not in resolved_map:
                            ...
                            self.log(f"   [MAPPED] {orig_alias} -> {ed_code} ({raw_jp}) ({raw_en}) | Start: {start_dt.strftime('%Y-%m-%d')}")
    ...
    def start_thread(self):
        ...
        self.run_btn_lbl.config(text="SCANNING...", bg=UITheme.SECONDARY)
    ...
    def execute_workflow(self):
        ...
        self.api_cache = {} 
        try:
            self.log("INITIATING CATEGORIZED SESSION...", clear=True)
            ...
            for ed_code, meta in target_map.items():
                ...
                self.log(f"--- Stock: {alias} ({ed_code}) ---")
            ...
            self.log("CATEGORIZED SYNC COMPLETED.")
    ...
    def download_and_extract(self, doc, master_dir, date_str, alias):
        ...
        jp_desc = str(doc.get("docDescription"))
        
        # --- NEW FIX: SANITIZE FILENAME ---
        # Replace slashes and other illegal characters with hyphens
        ...
        # --- CHANGE THIS LINE ---
        # Use safe_desc instead of the raw jp_desc for the filename
        ...
        if doc.get("pdfFlag") == "1":
            ...
            if r.status_code == 200:
                # Joining path with sanitized prefix
                file_path = os.path.join(stock_dir, f"{prefix}.pdf")
                    ...
                    with zipfile.ZipFile(zip_p, 'r') as z:
                        for m in z.namelist():
                            if m.endswith('.csv'):
                                ...
                                # Ensure the extracted CSV also uses the safe prefix
                                final_csv = os.path.join(stock_dir, f"{prefix}_{os.path.basename(m)}")
                                shutil.move(ext, final_csv)
                    os.remove(zip_p)
...
if __name__ == "__main__":
    root = tk.Tk(); app = EdinetB1CategorizedTool(root); root.mainloop()